...
function updateResults(results) {
  // Effacer le contenu actuel des résultats
  $("#resultat").empty();

  // Générer le contenu des résultats avec les nouvelles données
  for (var i = 0; i < results.length; i++) {
    var result = results[i];

    // Créer les éléments HTML pour afficher les résultats
    var container = $("<div>").addClass("result-container");
    var title = $("<h3>").text(result.marque + " " + result.modele);
    var details = $("<p>").text(
      "KM: " + result.kilometres + " - Année: " + result.annee
    );

    // Ajouter les éléments au conteneur des résultats
    container.append(title, details);

    // Ajouter le conteneur des résultats à la liste des résultats
